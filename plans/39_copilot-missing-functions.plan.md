...
- **Tool**: Register `estimates.updateWbsItems` in `lib/mcp/registry.ts`.
...
- **Side Effects**: In `lib/copilot/actions.ts`, add `estimates.updateWbsItems` to `SIDE_EFFECTS` causing `recalculateQuoteTotals` and `maybeRegenerateQuoteTerms`.
